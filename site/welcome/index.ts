...
function applyTemplates() {
  const templateKeys = Object.keys(officeInfo.template) as Array<
    keyof typeof officeInfo.template
  >;
...
export function setupReservationForm() {
  ...
  function constraintDepartureDate() {
    ...
    if (!departureDate.value || departureDate.value < minDepartureDate)
      departureDate.value = minDepartureDate;
  }
  ...
  on("input:show-site", (event?: Event) => {
    ...
    const sitePreviewImg =
      document.querySelector<HTMLImageElement>("#site-preview-img");
    if (!sitePreviewImg) return;

    const siteInfo = siteMap.find(
      (siteInfo) => siteInfo.site === parseInt(siteNumber)
    );
  ...
  function updateAvailableSites() {
    ...
    const sitesAvailable = sitesAvailableOnArrival.filter((s) =>
      isAvailable(s.availableDates, { arrivalDate, departureDate })
    );

    document
      .querySelectorAll(".if-no-vacancy")
      .forEach((element) =>
        element.classList.toggle("hidden", sitesAvailable.length > 0)
      );
    document
      .querySelectorAll(".if-vacancy")
      .forEach((element) =>
        element.classList.toggle("hidden", sitesAvailable.length === 0)
      );

    // black out all sites not available on departure
    const sitePickers = document.querySelectorAll<HTMLButtonElement>(
      ".site-picker-button"
    );
    sitePickers.forEach((sitePicker) => {
      const siteNumber = sitePicker.value;
      const siteInfo = sitesAvailable.find(
        (siteInfo) => siteInfo.site === parseInt(siteNumber)
      );
  ...
  function compute() {
    ...
    try {
      ...
      const total = siteNumbers.reduce((acc, siteNumber) => {
        ...
        if (!isAvailable(availableDates, { arrivalDate, departureDate }))
          throw log("site is not available");

        return acc + computeSiteCharge(days, dailyRate);
      }, 0);
      bindings["{{total-due}}"] = total ? asUsd(total) : "";
      document
        .querySelectorAll(".if-price")
        .forEach((element) => element.classList.toggle("hidden", !total));
    } catch (ex) {
      bindings["{{total-due}}"] = "";
      document
        .querySelectorAll(".if-price")
    ...
    const otherRates = document.querySelectorAll<HTMLDivElement>(
      ".site-picker-button .rate"
    );
    otherRates.forEach((rate) => {
      const siteNumber = rate.closest<HTMLButtonElement>(
        "button.site-picker-button"
      )?.value;
      if (!siteNumber) return log("site number is required");
      const dailyRate = siteMap.find(
        (siteInfo) => siteInfo.site === parseInt(siteNumber)
      )?.dailyRate;
    ...
    return siteMap
      .map((siteInfo) => {
        ...
        return `<button class="site-picker-button site_${site}" value="${site}" title="${about}">
            ${
              site
                ? `<div class="small available">Available</div><div class="small reserved">Reserved</div><div class="site-number large">Site ${alias}</div>`
                : ""
            }
            <nav class="grid grid-3 pad-1">
                ${
                  power
                    ? `<div class="power ${utility}"></div>`
                    : `<div class="nope ${utility}"></div>`
                }
                ${
                  water
                    ? `<div class="water ${utility}"></div>`
                    : `<div class="nope ${utility}"></div>`
                }
                ${
                  sewer
                    ? `<div class="sewer ${utility}"></div>`
                    : `<div class="nope ${utility}"></div>`
                }
...
function isAvailable(
  ...
  const dates = interpolateDates(
    dateRange.arrivalDate,
